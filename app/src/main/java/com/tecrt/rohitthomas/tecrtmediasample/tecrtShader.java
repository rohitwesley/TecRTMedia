...
//            + "}";
    ...
    public static final String introfilter
            ...
            + " vec4 O = texture2D(sTexture, .5+.5*p);\n"
            + " float N = 12., c = cos(6.28/N),s=sin(6.28/N), a = 3.14/4.*(.5+.5*sin(uTime)),d,A;\n"
            + " for (int i=0; i<20; i++) {\n"
            + " d = -dot(p-vec2(-1,1),vec2(sin(a),cos(a)));\n"
            + " A = smoothstep(.01,0.,d);\n"
            + " O.rgb += (1.-O.w) * A * vec3(1.-4.*smoothstep(.01,0.,abs(d)));\n"
            + " O.w = A;\n"
            + " p *= mat2(c,-s,s,c);\n"
            + " }\n"
            + " O *= smoothstep(1.,.99,length(p));\n"
